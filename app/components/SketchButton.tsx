...
const SketchButton: React.FC<SketchButtonProps> = ({ text, onClick, href }) => {
  
  // 1. Define the Keyframes for the animation
  const keyframes = `
    @keyframes sketch-idle {
    ...
    .sketch-button:hover { transform: rotate(-2deg) scale(1.05); }
  `;
  ...
  const buttonStyle: React.CSSProperties = {
    ...
    fontWeight: 'bold',
    color: '#333',
    animation: 'sketch-idle 1s infinite ease-in-out',
    transition: '0.3s ease-in-out',
  };

  const bgSvgStyle: React.CSSProperties = {
    position: 'absolute',
    top: '-20%',
    left: '-20%',
    width: '140%',
  ...
  const highlightSvgStyle: React.CSSProperties = {
    position: 'absolute',
    top: 0,
    left: 0,
    width: '100%',
  ...
  const ButtonContent = (
    <>
      ...
      <button className="sketch-button" style={buttonStyle} onClick={onClick}>
        <svg style={bgSvgStyle} viewBox="0 0 256 256" xmlns="http://www.w3.org/2000/svg">
        ...
        <svg style={highlightSvgStyle} viewBox="0 0 144.75738 77.18431" preserveAspectRatio="none">
